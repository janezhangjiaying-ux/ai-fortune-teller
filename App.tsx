...
const App: React.FC = () => {
  ...
  const [previousMode, setPreviousMode] = useState<ViewMode | null>(null); // 记录进入PROFILE模式前的页面
  ...
  const handleEnterProfile = () => {
    setPreviousMode(selectedMode); // 记录当前模式
    setSelectedMode('PROFILE'); // 切换到PROFILE模式
  };

  const handleExitProfile = (profile: UserProfile | null) => {
    handleCompleteOnboarding(profile);
    setSelectedMode(previousMode); // 恢复到之前的模式
  };
  ...
  if (!selectedMode) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-[#060608] transition-colors duration-500">
        <ModeSelection onSelect={handleModeSelect} onToggleTheme={toggleTheme} theme={theme} onResetProfile={handleResetProfile} onProfileSettings={handleEnterProfile} />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[#060608] text-slate-900 dark:text-slate-200 transition-colors duration-500 pb-32">
      ...
      <main className="max-w-4xl mx-auto p-4 space-y-8 flex flex-col min-h-[calc(100vh-140px)]">
        {selectedMode === 'HISTORY' ? (
          ...
          <div className="animate-in slide-in-from-top-4 duration-700">
            <OnboardingForm 
              ...
              mode={userProfile ? 'view' : 'onboarding'}
              onComplete={handleExitProfile} 
            />
          </div>

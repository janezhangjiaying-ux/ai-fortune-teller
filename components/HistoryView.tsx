...
const HistoryView: React.FC<HistoryViewProps> = ({ records, onDelete, onSelect, onBack }) => {
  ...
  const getRecordTitle = (record: HistoryRecord) => {
    ...
    if (record.type === 'HUANGLI') {
      ...
      return data.lunarDate || '老黄历';
    }
    return '历史记录';
  };

  return (
    <div className="w-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-4xl mx-auto">
      ...
      {records.length === 0 ? (
        <div className="w-full bg-white/50 dark:bg-slate-900/40 border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl p-16 text-center space-y-4">
        ...
        <div className="w-full space-y-4">
          {records.sort((a, b) => b.timestamp - a.timestamp).map((record) => (
            <div 
              key={record.id}
              className="w-full group bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/50 rounded-2xl p-4 flex items-center gap-4 hover:border-amber-500/30 transition-all cursor-pointer relative overflow-hidden shadow-sm dark:shadow-none"
              ...
              <div className="flex-grow min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 uppercase tracking-tighter">
                    {record.type === 'ASTROLOGY' ? '命理' : record.type === 'DREAM' ? '解梦' : record.type === 'HUANGLI' ? '黄历' : '占卜'}
                  </span>
                </div>
